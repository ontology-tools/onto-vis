...
for result in linksData:
    ...
    if "_outline" in download_url:
        ...
        if repo_name not in ["ADDICTO", "BCIO", "GMHO"]:
            ...
            source_repositories[
                'ADDICTO'] = "https://raw.githubusercontent.com/addiction-ssa/addiction-ontology/master/addicto.owl"  # source_url # todo: make this kluge exception for AddictO go away
            source_repositories[
                'BCIO'] = 'https://raw.githubusercontent.com/HumanBehaviourChangeProject/ontologies/master/Upper%20Level%20BCIO/bcio.owl'
            source_repositories[
                'GMHO'] = "https://raw.githubusercontent.com/galenos-project/mental-health-ontology/main/gmho.owl"
                ...
                if classId:
                    ...
                    if classId not in self.graphs[repo].nodes:
                        ...
                        if label:
                            ...
                            self.graphs[repo].add_node(classId,
                                                       # test add ID without label todo: is this necessary? Doesn't seem to hurt?
                                                       label=" \n",
                                                       **OntologyDataStore.node_props)
                else:
                    ...
                    for a in axioms:
                        ...
                        if isinstance(a.component, SubClassOf) and isinstance(a.component.sup, ObjectSomeValuesFrom):
                            ...
                            if isinstance(target, Class):
                                ...
                                targetLabel = self.releases[repo].get_annotation(str(target.first),
                                                                                 app.config['RDFSLABEL'])
                                if rel_name is None:
                                    pass
                                else:
